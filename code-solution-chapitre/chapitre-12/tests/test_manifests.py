...
def charger_multi(nom: str) -> dict:
    """`mlflow.yaml` contient trois documents YAML séparés par `---`."""
    docs = yaml.safe_load_all((K8S / nom).read_text(encoding="utf-8"))
...
# --- MLflow dans le cluster ---------------------------------------------------

@pytest.fixture(scope="module")
def mlflow_objets() -> dict:
    return charger_multi("mlflow.yaml")


def test_mlflow_declare_ses_trois_objets(mlflow_objets):
    """Sans PersistentVolumeClaim, l'historique d'expériences vit dans la couche
    jetable du container : un `kubectl delete pod` l'efface entièrement."""
    assert set(mlflow_objets) == {"PersistentVolumeClaim", "Deployment", "Service"}


def test_le_service_mlflow_porte_le_nom_attendu_par_la_configmap(mlflow_objets):
    """Le nom du Service EST le nom DNS interrogé par les Pods : la moindre
    faute de frappe et l'API ne trouve plus le Registry."""
    uri = charger("configmap.yaml")["data"]["MLFLOW_TRACKING_URI"]
    service = mlflow_objets["Service"]
    hote, port = uri.removeprefix("http://").split(":")
    assert service["metadata"]["name"] == hote
    assert service["spec"]["ports"][0]["port"] == int(port)


def test_un_seul_replica_mlflow(mlflow_objets):
    """Deux processus MLflow qui écrivent dans le même fichier SQLite le
    corrompent. La réplication attend PostgreSQL."""
    assert mlflow_objets["Deployment"]["spec"]["replicas"] == 1
    assert mlflow_objets["Deployment"]["spec"]["strategy"]["type"] == "Recreate"


def test_le_backend_store_pointe_sur_un_chemin_absolu(mlflow_objets):
    """`sqlite:///` (trois slashes) écrit dans un fichier relatif au répertoire
    courant du container, hors du volume : perdu au premier redémarrage."""
    conteneur = mlflow_objets["Deployment"]["spec"]["template"]["spec"]["containers"][0]
    args = " ".join(conteneur["args"])
    assert "--backend-store-uri=sqlite:////mlflow/mlflow.db" in args


def test_la_base_et_les_artefacts_sont_sur_le_volume(mlflow_objets):
    conteneur = mlflow_objets["Deployment"]["spec"]["template"]["spec"]["containers"][0]
    montage = conteneur["volumeMounts"][0]["mountPath"]
    args = " ".join(conteneur["args"])
    assert montage == "/mlflow"
    assert f"--artifacts-destination={montage}/artifacts" in args


def test_mlflow_ecoute_sur_toutes_les_interfaces(mlflow_objets):
    """Même piège qu'avec uvicorn au Chapitre 6 : lié à 127.0.0.1, le serveur
    ne répond qu'à l'intérieur de son propre container."""
    conteneur = mlflow_objets["Deployment"]["spec"]["template"]["spec"]["containers"][0]
    assert "--host=0.0.0.0" in conteneur["args"]
...
@pytest.mark.parametrize(
    "template",
    ["configmap.yaml", "deployment.yaml", "service.yaml", "hpa.yaml", "mlflow.yaml"],
)
...
def test_le_chart_embarque_la_configmap_et_le_serveur_mlflow():
    """Un chart qui n'emporte que Deployment/Service/HPA ne s'installe pas : le
    Pod reste en `CreateContainerConfigError` faute de ConfigMap, et l'URL
    `http://mlflow-service:5000` qu'elle publie ne pointe sur rien."""
    templates = {f.name for f in (CHART / "templates").glob("*.yaml")}
    assert {"configmap.yaml", "mlflow.yaml"} <= templates


def test_le_service_mlflow_du_chart_porte_le_nom_attendu_par_la_configmap():
    """Le nom du Service et l'hôte de MLFLOW_TRACKING_URI sont la même adresse,
    vue des deux bouts. Les faire diverger casse le chargement du modèle."""
    values = yaml.safe_load((CHART / "values.yaml").read_text(encoding="utf-8"))
    hote = values["config"]["mlflowTrackingUri"].split("//")[1].split(":")[0]
    assert hote == values["mlflow"]["serviceName"]


def test_le_chart_ne_contient_aucun_secret():
    """Un chart se versionne dans Git, un jeton d'accès jamais. Le Secret est
    créé hors du chart (`kubectl create secret`), qui n'en connaît que le nom."""
    for fichier in (CHART / "templates").glob("*.yaml"):
        contenu = fichier.read_text(encoding="utf-8")
        assert "kind: Secret" not in contenu, f"{fichier.name} embarque un Secret"
    values = yaml.safe_load((CHART / "values.yaml").read_text(encoding="utf-8"))
    assert values["secretName"], "le nom du Secret externe doit rester dans values.yaml"


def test_le_mlflow_du_chart_reste_en_un_seul_replica():
    """SQLite ne supporte pas deux écrivains : `replicas: 1` et la stratégie
    `Recreate` sont ce qui empêche la corruption silencieuse de la base."""
    contenu = (CHART / "templates" / "mlflow.yaml").read_text(encoding="utf-8")
    assert "replicas: 1" in contenu
    assert "type: Recreate" in contenu
